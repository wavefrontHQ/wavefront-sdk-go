...
func ExampleNewSender_options() {
	...
	sender, err := wavefront.NewSender(
		...
		wavefront.InternalMetricsEnabled(false),                         // Disable sending internal metrics
		wavefront.InternalMetricsTicker(time.NewTicker(30*time.Second)), // Report internal metrics every 30 seconds
